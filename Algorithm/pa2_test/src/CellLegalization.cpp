...
//         for (int j = accumulatedWidth[i-1]; j < acc; j++)
//             cost[j] = LLONG_MAX;
//         for (int j = acc; j <= W; j++)
...
vector<int> w, x, e;
vector<long long int> accumulatedWidth;
vector<long long int> old(600010, 0);
vector<long long int> cost(600010, 0);
